...
上述代码是对当前server组件要调用的APi所在的 组件 RouterTestModule 的命名空间， API文件（Target文件） 以及API 方法名 ， 这里定义的方法名，必须和 组件 RouterTestModule 对外暴露的API 方法名一一对应，（不带前缀，前缀在路由器中会自动加上，但是API 方法体重必须有前缀， 这也是为了在开发中一眼就能看出 这是组件通讯的API方法）
